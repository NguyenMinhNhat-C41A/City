...
namespace wolf
{
	...
	Texture *TextureManager::createCubemapTexture(const std::vector<std::string> &paths)
	{
		std::string key;
		...
		std::map<std::string, Entry *>::iterator iter = m_textures.find(key);
		if (iter != m_textures.end())
		{
			iter->second->m_refCount++;
			return iter->second->m_pTex;
		...
		// Create a new Texture object with the array of paths
		Texture *pTex = new Texture(paths);
		Entry *pEntry = new Entry(pTex);
		m_textures[key] = pEntry;
		return pTex;
	};
	//----------------------------------------------------------
