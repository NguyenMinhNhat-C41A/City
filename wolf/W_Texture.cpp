...
namespace wolf
{
	...
	//----------------------------------------------------------
	// Constructor, taking in filename to load and type
	//----------------------------------------------------------
	...
	void Texture::BindCubemapTex(int texUnit) const
	{
		glActiveTexture(GL_TEXTURE0 + texUnit);

		glBindTexture(GL_TEXTURE_CUBE_MAP, m_glTex);
	}
	...
	void Texture::LoadImagesAsCubemap(const std::vector<std::string> &paths)
	{
		if (paths.size() != 6)
		{
			std::cerr << "Error: Expected 6 paths for cubemap textures, got " << paths.size() << std::endl;
			return;
		}
		...
		{
			unsigned char *pData = stbi_load(paths.at(i).c_str(), &w, &h, &n, 0);
			if (!pData)
			{
				std::cerr << "Error loading sky texture at path: " << paths.at(i) << std::endl;
				continue;
			}
			...
			case 4:
				...
				break;
			default:
				std::cerr << "Unsupported texture format in file: " << paths.at(i) << std::endl;
				stbi_image_free(pData);
				continue;
			}

			glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, format, w, h, 0, format, GL_UNSIGNED_BYTE, pData);
			stbi_image_free(pData);
		}
	...
	void Texture::SetCubemapFilterMode(FilterMode minFilter, FilterMode magFilter)
	{
		if (magFilter == FM_Invalid)
			magFilter = minFilter;
