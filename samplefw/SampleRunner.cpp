...
void SampleRunner::destroyCurrentSample()
{
    const int prevSamplesSize = this->m_samples.size();
    const int prevSampleNumber = this->m_currSampleNumber;
    this->nextSample();

    delete this->m_samples[prevSampleNumber];
    if (this->m_currSampleNumber >= 0)
    {
        this->m_currSampleNumber -= 1;
    };
}
